...
function getArtistByIndex(array, index) {
  for (let i = 0; i < array.length; i++) {
    if (i === index) {
      console.log(`The artist at index ${array[i].id} is ${array[i].name}`);
    }
  }
}
...
function get20s(arr) {
  for (let i = 0; i < arr.length; i++) {
    let yearsStr = arr[i].years;
    let yearsArr = yearsStr.split(" - ");
    let born = yearsArr[0];
    let died = yearsArr[1];
...
console.log(addArtist(20, "Justin Mavity", "1986 - present", "student", "American", "Lorem ipsum dolor sit amet consectetur adipisicing elit. Architecto aut aliquid praesentium blanditiis alias similique nihil sequi repellat distinctio officia at corporis neque molestias ea debitis, sint velit. Laborum, ad?"));
